...
public class InventoryCleaner extends Module {
    ...
    public final List<Block> BLOCK_BLACKLIST = Arrays.asList(Blocks.enchanting_table, Blocks.chest, Blocks.ender_chest, Blocks.trapped_chest,
    ...
    public int weaponSlot = 36, pickaxeSlot = 37, axeSlot = 38, shovelSlot = 39;
    private final ArrayList<Integer> whitelistedItems = new ArrayList<>();
    private final TimerUtility timer = new TimerUtility();

    private final ModeValue ModeValue = new ModeValue("Mode", new String[]{"Normal", "InvOpen"}, "Normal");

    private final BoolValue Food = new BoolValue("Food", true, () -> true),
    ...
    private final NumberValue
            blockCap = new NumberValue("MaxBlock", 256.0, 0.0D, 256.0D, 8.0D, () -> true),
            delay = new NumberValue("Delay", 1.0D, 0.0D, 100.0D, 1.0D, sort::getValue),
            autoArmorDelay = new NumberValue("AutoArmorDelay",1.0D, 0.0D, 100.0D, 1.0D, autoArmor::getValue);
    ...
    @EventTarget
    public void onWorldReload(EventWorldReload e){
        timer.reset();
    }
...
//        if (checkLobby.getValue() && (Ro.getInstance().getServerUtils().isOnLobby())) return;
    ...
    public boolean shouldDrop(ItemStack stack, int slot) {
        ...
        if (stack.getDisplayName().toLowerCase().contains("speed")) {
            return false;
        }
        ...
        if (checkItems.getValue() && (stack.getItem() instanceof ItemBow && stack.getDisplayName().toLowerCase().contains("kit selector")) || (stack.getDisplayName().toLowerCase().contains("teleport")) ) {
            return false;
        }
        ...
        if (blockCap.getValue().intValue() != 0 && stack.getItem() instanceof ItemBlock && (getBlockCount() > blockCap.getValue().intValue() || BLOCK_BLACKLIST.contains(((ItemBlock) stack.getItem()).getBlock()))) {
            return true;
        }
