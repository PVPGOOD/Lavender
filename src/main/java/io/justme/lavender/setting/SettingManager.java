...
public class SettingManager {
    ...
    private final BoolValue gameEndValue = new BoolValue("自动关闭模块 (当游戏结束时)", true);
    private final MultiBoolValue gameEndMultiValue = new MultiBoolValue("对应的模块" , () -> getGameEndValue().getValue(),
    ...
    private final MultiBoolValue teamsCheckMultiValue = new MultiBoolValue("对应的模块", () -> getTeamsCheck().getValue() ,
            new BoolValue("KillAura ", true, () -> getTeamsCheck().getValue()),
            new BoolValue("AimAssist", true, () -> getTeamsCheck().getValue())
    );
    //antibot
    private final BoolValue antiBotCheck = new BoolValue("假人检测", true);
    private final ModeValue antibotCheckModeValue = new ModeValue("假人检测模式", new String[]{"Hypixel"}, "Hypixel", () -> getAntiBotCheck().getValue());
    private final MultiBoolValue antiBotCheckMultiValue = new MultiBoolValue("对应的模块", () -> getAntiBotCheck().getValue() ,
    ...
    //渲染
    //后期渲染
    private final BoolValue postRenderingValue = new BoolValue("后期渲染", true);
    private final MultiBoolValue postRenderingMultiValue = new MultiBoolValue("后期渲染设置", () -> getPostRenderingValue().getValue() ,
    ...
    private final NumberValue blurStrange =
            new NumberValue("模糊强度", 5, 1, 10, 1 , () -> getPostRenderingValue().getValue());
    private final NumberValue shadow =
            new NumberValue("阴影强度", 5, 1, 10, 1 , () -> getPostRenderingValue().getValue());
